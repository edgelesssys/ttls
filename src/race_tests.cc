...
TEST(Race, Client) {
  ...
  auto t1 = StartTestServer("9020", MBEDTLS_SSL_VERIFY_NONE);
  auto t2 = StartTestServer("9030", MBEDTLS_SSL_VERIFY_NONE);
  auto t3 = StartTestServer("9040", MBEDTLS_SSL_VERIFY_NONE);
  auto t4 = StartTestServer("9050", MBEDTLS_SSL_VERIFY_NONE);
...
TEST(Race, Server) {
  ...
  auto server_func = [&](int port) {
    ...
    auto t1 = StartTestClient(std::to_string(port), true);
